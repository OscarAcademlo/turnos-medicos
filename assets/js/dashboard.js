document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('backend/api/me.php')
        ...
        .then(data => {
            ...
            // Iniciar Wizard a Pantalla Completa si venimos de la agenda pública
            const urlParams = new URLSearchParams(window.location.search);
            const medicoIdParam = urlParams.get('agendar_medico_id');
            if (medicoIdParam) {
                setTimeout(() => {
                    iniciarWizardReserva(medicoIdParam);
                }, 100);
            }
...
// ==========================================
// LÓGICA DEL WIZARD DE RESERVA (PANTALLA COMPLETA)
// ==========================================
...
function iniciarWizardReserva(medicoId) {
    // 1. Ocultar todo y mostrar el wizard
    document.querySelectorAll('.nav-link').forEach(l => l.classList.remove('active'));
    document.getElementById('content-dashboard').classList.add('d-none');
    document.getElementById('content-wizard-agendar').classList.remove('d-none');
    document.getElementById('page-title').textContent = 'Solicitar Turno';

    wizardData.medicoId = medicoId;
    
    // 2. Fetch de datos del médico para llenar la barra superior
...
function wizardCargarObrasSociales() {
...
function wizardSelectCobertura(id, nombre) {
...
function wizardCargarPlanes(osId) {
    ...
    fetch(`backend/api/get_planes_by_os.php?os_id=${osId}`)
        ...
        .then(data => {
            ...
            if(!data.planes || data.planes.length === 0) {
                // No hay planes, ir directo a calendario
                document.getElementById('wizard-step-3').classList.add('d-none');
                wizardGoToStep4();
                return;
            }
...
function wizardRenderCalendarioMock() {
    ...
    for(let i=1; i<=14; i++) {
        ...
        let cupos = Math.floor(Math.random() * 10) + 1; // 1 a 10 cupos
...
function wizardShowHorarios(cupos) {
...
// Botones de retroceso en la barra superior
document.getElementById('btn-cambiar-profesional').addEventListener('click', () => {
    // Si quiere cambiar el profesional, lo mandamos de vuelta al index
